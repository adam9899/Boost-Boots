    ...
    public boolean handle(Supplier<NetworkEvent.Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if(serverplayer!= null) {
                ...
                if (serverEventHandler.airBorneLastTick.get(serverplayer)
                        && !serverEventHandler.flag.get(serverplayer)
                        && BoostUtils.hasBoostBoots(serverplayer)) {

                    serverplayer.abilities.allowFlying = true;

                    float f1 = serverplayer.rotationYaw * ((float) Math.PI / 180F);
                    serverplayer.setMotion((-MathHelper.sin(f1) * 0.2F), 0.6f, (MathHelper.cos(f1) * 0.2F));
                    ...
                    for (ItemStack equippedBoot : BoostUtils.getEquippedBoots(serverplayer)) {
                        equippedBoot.damageItem(1, serverplayer, (playerEntity) -> {
